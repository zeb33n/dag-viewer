...
use dot_parser::ast::{Graph, either::Either::{Left, Right}};
...
impl Model {
    ...
    fn get_logical_nodes(graph: &Graph<(dot_parser::ast::ID<'_>, dot_parser::ast::ID<'_>)>) -> Vec<Node> {
        let mut r: Vec<Node> = vec![];
        let stmts = &graph.stmts;
        for s in stmts {
            match s {
                dot_parser::ast::Stmt::NodeStmt(node_stmt) => {
                    r.push(Node { label: node_stmt.name().to_string(), dependents: vec![] });
                },
